...
7. **Secret-Klartext in dieser Datei redigiert** (Zeile 211/1770): `whsec_ROTATED_2026-08-02_SEE_PHASENSTATUS` durch `whsec_…[REDIGIERT]`-Platzhalter ersetzt, damit der Wert nicht mit jedem künftigen Commit erneut in der Arbeitskopie steht. **Das entfernt ihn NICHT aus der Git-Historie** (bereits im Commit `ea1c1bd` und allen späteren Commits enthalten, die diese Datei berühren) — dafür bräuchte es `git filter-repo` + Force-Push, eine destruktive, history-umschreibende Operation, die laut Sicherheitsregeln explizite Freigabe braucht und hier bewusst NICHT ausgeführt wurde. **Falls das Secret noch aktiv einem Stripe-Webhook-Endpoint zugeordnet ist, bleibt es trotz Redaktion kompromittiert, bis Josip es im Stripe-Dashboard rotiert und den neuen Wert als Cloudflare-Worker-Secret einträgt** (kein `wrangler secret put`-Äquivalent per MCP verfügbar, siehe CLAUDE.md §2.2).
...
**Noch offen:**
1. Secret-Rotation (Stripe + Cloudflare-Worker-Secret) — nur Josip, kein passendes MCP-Tool.
2. Git-History-Bereinigung des Secrets — destruktiv, braucht Josips ausdrückliche Freigabe und einen Force-Push.
3. Leaked-Password-Protection im Supabase-Dashboard aktivieren — nur Josip oder eine Sitzung mit Browser-Zugriff.
