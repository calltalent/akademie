...
## Produktions-Bug: Betreiber-Portal per Wartungsmodus eines Mandanten ausgesperrt (08.08.2026, Josips Fund)

Josip meldete: `academy.calltalent.ai/portal/mandanten/eff4aa20-...` lädt nicht mehr, Browser zeigt "This page couldn't load". `curl` gegen die Produktions-URL bestätigt: HTTP 503, ausgeliefert wird die App-eigene Wartungsseite ("Wir sind gleich zurück"), nicht ein Netzwerk-/Build-Fehler.

**Ursache:** `academy.calltalent.ai` ist zugleich `custom_domain` des Mandanten „calltalent" (`tenants.id = eff4aa20-9295-47f1-be4f-18fe049336c6`, per SQL bestätigt) UND — laut Josip — nicht der konfigurierte `NEXT_PUBLIC_PORTAL_HOST`. Ein Aufruf von `/portal/...` auf diesem Host löst deshalb in `middleware.ts` trotzdem den Mandanten „calltalent" auf (Next.js bedient `src/app/portal/...` rein pfadbasiert, unabhängig vom Host — der eigentliche Zugriffsschutz für das Betreiber-Portal ist `checkPlatformAccess()` INNERHALB der Seite, keine Host-Prüfung in der Middleware). Der Mandant „calltalent" hatte `settings.maintenance_enabled = true` gesetzt, und `isMaintenanceBypassPath()` (`src/lib/tenant/routing.ts`) enthielt `/admin/...` als Ausnahme (damit ein Owner sich nach dem Aktivieren noch abmelden/den Schalter wieder ausschalten kann), aber **`/portal/...` fehlte** — exakt dieselbe Fehlerklasse, nur eine Ebene höher (Plattform- statt Mandanten-Admin). Josip wurde dadurch vom Wartungsmodus EINES einzelnen Mandanten aus der eigenen, mandantenübergreifenden Betreiber-Verwaltung ausgesperrt.

**Fix:** `isMaintenanceBypassPath()` um `/portal` und `/portal/*` ergänzt (`src/lib/tenant/routing.ts`), drei neue Testfälle in `routing.test.ts` (inkl. der genauen betroffenen URL aus dem Fund). `npx vitest run`: 634 Tests grün. `npx tsc --noEmit`/`npm run lint`: sauber.

**Noch offen:** Dieser Fix braucht ein Deploy, um live zu wirken (`npm run deploy`, Josips Aufgabe). Bis dahin bleibt `/portal/...` auf `academy.calltalent.ai` blockiert, solange `maintenance_enabled` für den Mandanten „calltalent" gesetzt ist — sofortige Abhilfe (ohne Deploy) wäre ein direktes Zurücksetzen des Flags in der DB, dafür erst Josips Bestätigung eingeholt (siehe Chat), da eine Produktionsdatenänderung.

## Schichtplan — Block S5a: Regionen und Mandanten-Einstellung (08.08.2026, builder)

Umsetzung nach dem freigegebenen architect-Plan (`plane-und-erstelle-mit-floofy-curry-agent-a2931f285e454e395.md`, Abschnitt 11 „S5a — Regionen und Mandanten-Einstellung"). Scope: acht Feiertags-Regionscodes einführen, Mandanten-Einstellung „Feiertagsregionen" (eigene Karte, eigene Server Action, eigene Migration geschrieben-aber-nicht-angewendet). Vorbereitung für die KI-Feiertagsrecherche der Blöcke S5b (Job-Verarbeitung) und S5c (Oberfläche, Ablösung des alten Imports) — beide bewusst NICHT Teil dieses Blocks.

**Erledigt:**

1. `src/lib/calendar/schema.ts`: `CALENDAR_HOLIDAY_COUNTRIES`/`CalendarHolidayCountryCode` (drei Werte) zu `CALENDAR_HOLIDAY_REGIONS`/`CalendarHolidayRegionCode` (acht Werte: `DE`, `AT`, `CH`, `HR`, `RS`, `BA_FBIH`, `BA_RS`, `BA_BRCKO`) umbenannt. `calendarHolidayImportSchema` referenziert jetzt `CALENDAR_HOLIDAY_REGIONS`, bleibt bis Block S5c bestehen. Neu: `tenantHolidayRegionsSchema` (`z.array(z.enum(CALENDAR_HOLIDAY_REGIONS))`) und die reine Funktion `parseTenantHolidayRegions(raw: unknown)` — dedupliziert, erlaubt leeres Array, wirft bei einem unbekannten Code.
2. `src/lib/calendar/holidays.ts`: NUR Dateikopf-Kommentar ergänzt (neue Rolle als nachgelagerter Abgleich ab Block S5b). Code, `CalendarHolidayCountry`-Typ und `holidays.test.ts` unverändert — bewusst enger als `CalendarHolidayRegionCode` (nicht synchron halten, siehe Kommentar dort).
3. `src/lib/tenant/types.ts`: `PublicTenant["settings"].shift_calendar_holiday_regions?: CalendarHolidayRegionCode[]` ergänzt, JSDoc nach bestehender Konvention (Polarität fehlend/leer = keine Region gewählt, Schreibzugriff nur über die neue Server Action).
4. `src/lib/tenant/actions.ts`: neue Server Action `updateTenantHolidayRegions()` — `requireAdminTenant()` → Gate `shift_calendar_enabled === true` → `parseTenantHolidayRegions(formData.getAll("holidayRegions"))` → MERGE-Schreibweise (niemals Overwrite) → `revalidatePath("/admin/einstellungen")` + `revalidatePath("/admin/schichtplanung")`. `updateTenantSettings()` unverändert, eigene Action bewusst getrennt (Datenverlust-Risiko bei ausgeblendeter Karte, siehe Kopfkommentar).
...
1. **`calendar-absences-panel.tsx` importiert NICHT `CALENDAR_HOLIDAY_REGIONS`, sondern eine neue lokale Konstante `IMPORT_COUNTRIES` (drei Werte, Typ `CalendarHolidayCountry` aus `holidays.ts`).** Der Auftrag verlangte wörtlich, den Import „auf CALENDAR_HOLIDAY_REGIONS umzustellen", aber im selben Satz auch, dass das Dropdown weiterhin ausschließlich die drei alten Länder zeigt. Beides gleichzeitig über einen Import von `CALENDAR_HOLIDAY_REGIONS` (acht Werte) hätte entweder ein 8-Werte-Dropdown ergeben (verletzt die zweite Vorgabe) oder eine unbegründete `.slice(0,3)`-Annahme über die Array-Reihenfolge gebraucht. Die lokale Dreier-Konstante mit dem bereits bestehenden, unveränderten `CalendarHolidayCountry`-Typ aus `holidays.ts` erfüllt beide Vorgaben ohne Ordnungsannahme: Dropdown zeigt weiterhin exakt DE/AT/CH, der Typ deckt sich exakt mit dem, was `buildHolidays()` tatsächlich verarbeitet.
2. **`src/lib/calendar/actions.ts`: `importCalendarHolidays()` bekam eine zusätzliche Laufzeitprüfung** (`if (data.country !== "DE" && data.country !== "AT" && data.country !== "CH") return { ok:false, error:"..." }` vor dem `buildHolidays()`-Aufruf). Nicht explizit im Auftrag genannt (die Datei stand nicht in der S5a-Dateiliste), aber zwingend: `calendarHolidayImportSchema.country` ist seit der geforderten Umstellung auf `CALENDAR_HOLIDAY_REGIONS` acht statt drei Werte breit, `buildHolidays()` erwartet weiterhin nur `CalendarHolidayCountry` (DE/AT/CH) — ohne die Prüfung hätte `npx tsc --noEmit` mit einem Typfehler gescheitert. Die Prüfung narrowt `data.country` per TypeScript-Kontrollfluss auf genau die drei Werte, die danach an `buildHolidays()` gehen, und liefert nebenbei eine sprechende Fehlermeldung, falls (derzeit von keiner UI erreichbar) doch einmal einer der fünf neuen Regionscodes an diese ältere Action geschickt würde.
3. **`errorUnknownRegion` (i18n) wird von `updateTenantHolidayRegions()` aktuell nicht ausgelöst, solange ausschließlich über das Checkbox-Formular gesendet wird.** `parseTenantHolidayRegions()` wirft bei jedem einzelnen ungültigen Code — das HTML-Formular kann aber gar keine ungültigen Werte erzeugen (feste `value`-Liste aus `CALENDAR_HOLIDAY_REGIONS`). Der Fehlertext bleibt trotzdem definiert und wird ausgelöst, sobald jemand die Server Action direkt mit manipulierten `FormData` aufruft (Verteidigung in der Tiefe) — bewusst kein totes i18n-Wissen, sondern ein Pfad, der nur über die UI aktuell nicht erreichbar ist.
...
**Übergabe an `tester`**: RLS-/Vitest-Verifikation dieses Blocks ist bereits grün (siehe Verifikation oben); der nächste sinnvolle Prüfschritt ist erst nach Block S5b sinnvoll (Migration muss zuerst live sein, bevor eine RLS-Gegenprobe für `kind='holiday_research'` überhaupt möglich ist). Für S5a selbst: `npx vitest run` als Regressionsnachweis, keine weiteren Schritte nötig, bevor S5b beginnt.

## Schichtplan — Block S5b: KI-Weg ohne Oberfläche (08.08.2026, builder)

Umsetzung nach dem freigegebenen architect-Plan (`plane-und-erstelle-mit-floofy-curry-agent-a2931f285e454e395.md`, Abschnitt 11 „S5b — KI-Weg ohne Oberfläche"). Scope: kompletter `src/lib/calendar/ai/holidays/`-Stack (Schema, Prompt-Bau, Abgleich, Konflikterkennung, Claude-Aufruf, Job-Verarbeitung, Server Actions, Leseabfragen), Einbindung in `usage.ts`/`process/route.ts`, zwei neue Vitest-Dateien. Kein UI (Block S5c), keine Migrationsanwendung und kein echter Claude-Testlauf in diesem Block — beide bleiben Aufgabe des Orchestrators im Anschluss, wie im Bauauftrag ausdrücklich festgelegt.

**Erledigt:**

1. `src/lib/calendar/ai/holidays/schema.ts` (NEU): `holidayResearchRequestSchema` (`year` 2020–2100, `regions` 1–8 Einträge aus `CALENDAR_HOLIDAY_REGIONS`), `holidayModelShiftSchema`/`holidayModelOutputSchema` (Claude-Antwortformat, Region als Klartext-Code, kein Pseudonym-Mapping nötig — Regionscodes sind keine personenbezogenen Daten), `HOLIDAY_CHECKS`/`HOLIDAY_CONFLICTS`, `holidayDraftRowSchema`, `holidayResearchOutputSchema`, `holidayApplyRowSchema`/`holidayApplySchema` (bewusst OHNE `region`-Feld — Region kommt nie vom Client, gleicher Sicherheitsgrund wie `workerId` bei `shiftPlanApplyRowSchema`).
2. `src/lib/calendar/ai/holidays/prompt.ts` (NEU, rein, kein I/O, kein `server-only`): `REGION_LABELS` (deckt sich mit `messages/de.json` `admin.shiftCalendar.absences.region.*`), `buildHolidayResearchPrompt()` — enthält für jede angeforderte Region den Klarnamen und einen ausdrücklichen Satz, der `RS` (Staat Serbien) von `BA_RS` (Republika Srpska, BiH-Entität) unterscheidet; KEIN Freitextfeld eingebettet, deshalb KEIN `UNTRUSTED_DATA_INSTRUCTION`-Marker (Angriffsfläche für Prompt-Injection entfernt statt gemindert). `mapHolidayDraftRows()` verwirft eine vom Modell gemeldete, aber nicht angeforderte Region und protokolliert das in `unresolvedNotes`, statt den ganzen Lauf zu verwerfen. `crossCheckHolidays()` importiert `buildHolidays()` aus `holidays.ts` als NACHGELAGERTEN Abgleich (nicht Prompt-Zutat): `confirmed`/`unverified` für `DE`/`AT`/`CH`, `null` für alle anderen Regionen; meldet zusätzlich von `buildHolidays()` bekannte, aber vom Modell ausgelassene Tage als Klartext-Hinweis in `missingNotes` (bewusst NICHT automatisch als Zeile ergänzt, siehe architect-Plan Abschnitt 2). `detectHolidayConflicts()` markiert `outside-year` vor `existing` vor `duplicate`, exakt das Prioritätsmuster von `detectConflicts()` (S4-Vorbild).
3. `src/lib/calendar/ai/holidays/pipeline.ts` (NEU): `callHolidayResearchModel()` — ein Sonnet-Aufruf über `createAnthropicClient()`, `parseStepResponse()` (`generator/parse.ts`), genau ein Retry bei Validierungsfehler, `max_tokens: 8000` (konservativ bemessen für bis zu acht Regionen × ~15 Feiertage). Kein Unit-Test (echter API-Aufruf), gleiche Begründung wie beim S4-Vorbild.
4. `src/lib/calendar/ai/holidays/process.ts` (NEU): `processNextHolidayResearchJob()` — CAS-Sperre (`STALE_RUNNING_MS = 3 Min.`), Job GEFILTERT NUR AUF `job.tenant_id` (niemals aus `job.input`, S4-Regel), `holidayResearchRequestSchema.safeParse()`, `enforceQuota(tenantId,"course_gen")` fail-closed VOR dem Claude-Aufruf, lädt bereits vorhandene `kind='holiday'`-Zeilen des Jahres für `detectHolidayConflicts()`, Ablauf `callHolidayResearchModel()` → `mapHolidayDraftRows()` → `crossCheckHolidays()` → `detectHolidayConflicts()` → `updateAiJob(status:"done", output, model, tokensIn, tokensOut)`. Jeder Fehlerfall bekommt `status:"error"` mit klarer deutscher Meldung, volles Detail nur ins Server-Log.
5. `src/lib/calendar/ai/holidays/actions.ts` (NEU, `"use server"`): `startHolidayResearchJob({year})` — `requireShiftPlannerTenant()` + explizites `isAdmin`-Gate, `checkRateLimit("ki-holidays",{maxRequests:3,windowSeconds:86400,extraKey:tenant.id})`, zod-Validierung NUR für `year` (`.pick({year:true})` auf `holidayResearchRequestSchema`, keine Duplizierung), Regionen SERVERSEITIG aus `tenant.settings.shift_calendar_holiday_regions` gelesen (leer → „Für diese Akademie ist noch keine Region ausgewählt."), Insert in `ai_jobs`. `applyHolidayResearch(jobId, rows)` — lädt Job, validiert `status==="done"`, `draftRowsById.get(clientRow.id)` löst die ECHTE Region auf (nie vom Client), EINE Zeile = EIN `.insert()` in `calendar_absences` (`worker_id:null, kind:'holiday', starts_on=ends_on=date, note:"${name} (${regionLabel})", status:'approved'`), Fehler pro Zeile gesammelt in `failures[]`, KEIN `try` um die ganze Schleife, `appliedRowIds` als Doppelklick-Schutz über `updateAiJob()` (Admin-Client). `deleteHolidayResearchJob(jobId)` — Admin-Client (keine DELETE-Policy auf `ai_jobs`), tenant-geprüft VOR dem Admin-Client-Zugriff.
6. `src/lib/calendar/ai/holidays/queries.ts` (NEU): `getHolidayResearchJobs()`/`getHolidayResearchJob()` — laufen über den regulären RLS-Client, lösen Regionscodes in der Detailabfrage zu `regionLabel` auf und markieren `alreadyApplied` aus `appliedRowIds`.
7. `src/lib/ai/usage.ts`: `kind`-Union in `recordAiJob()` um `"holiday_research"` erweitert (gleiche Stelle, Kommentarstil wie bei `"shift_plan"`).
8. `src/app/api/admin/ki/process/route.ts`: `processNextHolidayResearchJob()` als DRITTER Eintrag im bestehenden `Promise.all` (neben Kurs-Generator und Schichtplanung), Antwortobjekt um `holidayResearch` erweitert, Kopfkommentar von „zwei“ auf „drei unabhängige Warteschlangen“ aktualisiert.
...
## Schichtplan — Block S5c: Oberfläche, Ablösung des alten Imports, e2e, Security-Review (08.08.2026, builder + Orchestrator)

Umsetzung nach dem freigegebenen architect-Plan (`plane-und-erstelle-mit-floofy-curry-agent-a2931f285e454e395.md`, Abschnitt 11 „S5c — Oberfläche und Ablösung des alten Imports"). Letzter Block des Features: Review-Oberfläche, Ablösung des S1/S2-Feiertagsimports, e2e-Suite, Security-Review, Dokumentation. `builder`-Agent lief in ein Sitzungslimit (dritte Unterbrechung dieser Art in der Session, nach S3/S4) — Code-Teil war zu diesem Zeitpunkt bereits vollständig (`tsc`/`vitest`/`lint` sauber), nur der letzte e2e-Lauf und die Abschlussschritte fehlten; vom Orchestrator direkt zu Ende gebracht statt eines weiteren Agenten-Neustarts.

**Erledigt:**

1. `src/components/admin/calendar-holidays-ki-panel.tsx` (NEU) — Auslöse-Formular (nur Jahr, Regionen kommen serverseitig) + Auftragsliste mit 5-Sekunden-Auto-Refresh, 1:1-Muster aus `calendar-ki-panel.tsx` (S4-Vorbild).
2. `src/components/admin/calendar-holidays-ki-review.tsx` (NEU) — Review-Tabelle (Region/Datum/Bezeichnung editierbar/Abgleich-Badge/Konflikt-Badge), KEIN `router.refresh()` nach Übernahme. **Abweichende Vorauswahl-Logik ggü. S4** (architect-Plan Abschnitt 5.3, bewusst): nur Zeilen mit `conflict === null` starten ausgewählt, Konfliktzeilen starten abgewählt — Feiertage sind Rechtstatsachen mit Entgeltfolge (§ 2 EFZG), eine unbedachte Übernahme wirkt mandantenweit.
3. `calendar-absences-panel.tsx`: alter Feiertagsimport-Block (`<select>` + `importCalendarHolidays()`) vollständig entfernt und durch die KI-Karte/Review ersetzt. Ohne gewählte Region: Hinweistext + Link zu den Einstellungen statt Formular.
4. `importCalendarHolidays()` (`calendar/actions.ts`) und `calendarHolidayImportSchema` (`calendar/schema.ts`) entfernt — vollständig durch den S5b-Job-Weg abgelöst.
5. `schichtplanung-tabs.tsx`/`(planung)/admin/schichtplanung/page.tsx`: neuer Query-Parameter `holidayJob`, `AbsencesData` um `holidayRegions`/`holidayJobs`/`holidayJobDetail` erweitert, nur für `isAdmin` geladen (Feiertagsrecherche ist admin-only).
6. i18n: `admin.shiftCalendar.absences.holidayKi.*` ersetzt `holidayImport.*` (entfernt), `absences.region.*` (aus S5a) ersetzt `absences.country.*` (entfernt) — alle drei Sprachen parallel gepflegt.
7. `e2e/schichtplan-s5-feiertage.spec.ts` (NEU, 9 serielle Fälle nach architect-Plan Abschnitt 9.3) — kein echter Claude-Aufruf (Entwurf wird per Admin-Client geseedet), Fall 9 RLS-Gegenprobe für `member` UND `trainer` gegen `kind='holiday_research'`.
8. **Dokumentierte Abweichung, zwingend**: `e2e/schichtplan-s2.spec.ts` Fall 7 ("Feiertagsimport DE 2026") ENTFERNT — testete ein Formular, das seit diesem Block nicht mehr existiert. Verstößt technisch gegen die sonst geltende Regel "S1-S4-Specs bleiben unverändert", war aber zwingend: ein Test auf entferntes UI kann nicht sinnvoll weiterbestehen. Nachfolge-Abdeckung vollständig in `schichtplan-s5-feiertage.spec.ts`.
9. Verifikation: `npx tsc --noEmit` → 0 Fehler. `npm run lint` → 0 Fehler/Warnungen. `npx vitest run` → **41 Dateien, 672 Tests, alle grün** (676 vorher, netto −4 durch den Wegfall der `calendarHolidayImportSchema`-Tests, die durch mehr `parseTenantHolidayRegions()`-Fälle in S5a bereits mehr als ausgeglichen waren).

**Ein e2e-Testbug gefunden und behoben (kein Produktfehler):** Der Abwesenheiten-Reiter zeigt zwei verschiedene `<ul><li>`-Listen gleichzeitig — die Liste bestehender Feiertage (`calendar_absences`, zeigt Daten wie „2099-01-01" als Text) und die neue KI-Auftragsliste. Ein ungescopter `page.locator("ul li").filter({hasText: YEAR})` traf zuerst die FALSCHE, bestehende Liste (DOM-Reihenfolge: Feiertage-Liste steht vor der KI-Karte) — bekannte Fehlerklasse, bereits bei S2/S4 aufgetreten. Fix: neue Hilfsfunktion `kiPanelSection()` scopt auf den Container unterhalb der KI-Überschrift (`ancestor::div[2]`, da die Überschrift in einem inneren Wrapper-`<div>` steckt). Betraf Fälle 4/5/8. Nach dem Fix: **`e2e/schichtplan-s5-feiertage.spec.ts`: 9/9 grün**, **`npx playwright test schichtplan`: 37/37 grün** (5+7+8+9+9 über alle fünf Spec-Dateien S1–S5).

**security-reviewer-Durchlauf (kompletter Block S5, a+b+c zusammen): keine KRITISCH/HOCH-Funde.** Alle sieben zur Gegenprüfung vorgelegten Architekturentscheidungen bestätigt korrekt umgesetzt (Datenminimierung, `region` nie vom Client bei der Übernahme, Regionen serverseitig aus Mandanten-Einstellungen, RLS-Verengung mit echtem e2e-Gegenbeweis statt bloßer Behauptung, MERGE+serverseitiges Feature-Gate bei `updateTenantHolidayRegions()`, Rate-Limit vor Jobstart, Kontingent fail-closed vor dem Claude-Aufruf). Drei Funde NIEDRIG:
1. Rate-Limit (3/Tag) nur auf Server-Action-Ebene, nicht per DB-Constraint — ein Mandanten-Admin könnte es per Direkt-Client umgehen, der Schaden bleibt aber auf das eigene Kontingent begrenzt (`enforceQuota()` deckelt trotzdem), gleiches vorbestehendes Muster wie bei `kind='shift_plan'`. Kein Fix in diesem Block.
2. Veralteter Migrationskopf-Kommentar ("nur geschrieben, nicht angewendet") — **sofort behoben** (Kommentar auf den tatsächlichen Live-Status aktualisiert, `supabase/migrations/20260808145112_calendar_holiday_research.sql`).
...
## Schichtplan — Block S6: Freelancer-Drag-and-Drop im Wochenraster (09.08.2026, builder + tester + security-reviewer)

Josips Auftrag: Freelancer sollen per Klick/Ziehen direkt im Wochenraster Zeiten ändern oder hinzufügen können. Zwei `AskUserQuestion`-Runden haben die Architektur geklärt (siehe architect-Plan `plane-und-erstelle-mit-floofy-curry.md`): (1) nur Freelancer, nicht der Admin-Bereich; (2) Ziehen an einer bestehenden Schicht schreibt NICHTS direkt — befüllt nur das bestehende „Zeit ändern"-Formular vor, Admin-Freigabe bleibt Pflicht; (3) Ziehen in einem offenen Zeitfenster darf einen Teil-Zeitraum buchen, ein reiner Klick bucht wie bisher den ganzen Slot. Scope-Einschränkung V1: nur vertikales Ziehen (Zeitänderung), keine Tagesänderung per Ziehen über Spalten hinweg.

**Erledigt:**

1. `src/lib/calendar/drag.ts` (NEU) — reine Hilfsfunktionen `snapToQuarterHour()`/`clampRange()`/`minutesFromPointerY()`, isoliert testbar ohne Pointer-Event-Simulation (`drag.test.ts`, 11 Fälle).
2. `src/lib/calendar/schema.ts` — `calendarSelfBookingSchema` um optionale `startTime`/`endTime` erweitert (beide-oder-keins, nicht gleich), rückwärtskompatibel — ein Aufruf mit nur `slotId` bleibt exakt das Altverhalten (voller Slot).
3. `src/lib/calendar/actions.ts` — `bookOwnShift()` berechnet bei gesetzten `startTime`/`endTime` den Zeitraum über `berlinDateTimeToUtc()` aus dem Slot-eigenen Kalendertag, prüft serverseitig vor dem Insert, dass der Bereich innerhalb der Slotgrenzen liegt (freundliche Vorprüfung; der bestehende Trigger `calendar_slot_capacity_guard()`/Fehlercode `CT002` bleibt die tatsächliche Sicherheitsgrenze — **keine neue Migration nötig**, der Trigger deckte Teil-Zeiträume schon vor S6 ab, da bisher nie einer eingefügt wurde).
4. `src/components/learn/shift-calendar-view.tsx` — jetzt `"use client"` (Pointer Events). Bestehende Ausgabe bei fehlenden neuen Props bleibt byte-identisch (S1/S2/S3-Testselektoren unverändert grün). Neu, alles optional: `openSlots`/`onBookSlot`/`bookingPending`/`onProposeChange`. Offene Zeitfenster als gestrichelte Blöcke im Raster (Klick = ganzer Slot, Ziehen = Teil-Zeitraum, Bewegungsschwelle 4px unterscheidet beides). Eigene Schicht-Blöcke bekommen bei `onProposeChange` Rand-Ziehzonen (Größe ändern) und Körper-Ziehen (verschieben) — ruft nur die Callback-Funktion auf, schreibt nie direkt. Dritter Legendenpunkt „Offenes Zeitfenster" nur wenn welche vorhanden sind.
5. `src/components/learn/shift-change-request-panel.tsx` — neue optionale Props `externalRequest`/`onExternalRequestHandled`, befüllen nur `ChangeForm`s Startwerte (`initialDate`/`initialStartTime`/`initialEndTime`) — der eigentliche Submit ruft weiterhin ausschließlich das unveränderte `createShiftChangeRequest()` auf.
6. `src/components/learn/shift-calendar-board.tsx` (NEU) — Orchestrator, schaltet Raster und Änderungsanfrage-Panel zusammen. Bei `isFreelancer === false` rendert er exakt das alte Paar ohne neue Props — Festangestellte sehen keine Verhaltensänderung.
7. `src/app/(portal)/schichtplan/page.tsx` — rendert `<ShiftCalendarBoard>` statt der getrennten Komponenten, rechnet `openSlots` in `ShiftCalendarOpenSlotBlock[]` um. `OpenSlotsPanel`/`TargetHoursForm` bewusst unverändert an ihrer Stelle belassen (additiv, Tastaturweg bleibt primär — Präzedenzfall `video-trimmer.tsx`).
8. i18n: `learn.shiftCalendar.dragBoard.{dragHint,openSlotLegend,openSlotAriaLabel,openSlotFullAriaLabel}` in allen drei Sprachen, Paritäts-Test grün.
9. **Dokumentierte Abweichung des builders vom Plan (zwei Punkte, beide begründet):** (a) `ShiftCalendarOpenSlotBlock` bekam zusätzlich ein `timeRange`-Feld (nicht im Plan-Wortlaut) — verhindert eine "00:00"-Anzeige bei Slots, deren gekappte Minuten sonst falsch formatiert würden, spiegelt `ShiftCalendarShift.timeRange`. (b) `ShiftCalendarView` filtert bereits selbst gebuchte offene Zeitfenster aus der Rasteranzeige (statt sie deaktiviert anzuzeigen) — verhindert eine Locator-Kollision in `schichtplan-s2.spec.ts` Fall 4, dessen Regex sonst auch auf einen noch sichtbaren „offen"-Block träfe.
10. Verifikation: `npx tsc --noEmit` → 0 Fehler. `npm run lint` → 0 Fehler. `npx vitest run` → **688/688 grün** (16 neue Tests, nichts gebrochen). `npx playwright test schichtplan` → **41/41 grün** über alle sechs Spec-Dateien (S1–S6), inkl. neuer `schichtplan-s6-drag.spec.ts` (4 Fälle: Teil-Buchung per Ziehen, Voll-Buchung per Klick als Paritäts-Regression, Schicht-Ziehen befüllt nur das Formular ohne Schreibung, Festangestellter sieht keine Drag-UI).
11. **Ein Testbug im neuen Spec gefunden und behoben** (kein Produktfehler): Fall 3 seedete die Vergleichsschicht fest auf „morgen" — an einem Sonntag (heutiges Testdatum) fällt das in die nächste ISO-Woche und damit außerhalb der Standardansicht. Neue Helfer `isoWeekdayFromDateOnly()`/`otherDayInSameWeekIso()` wählen stattdessen garantiert einen Tag derselben Woche.
12. Live-Check im Browser (Freelancer-Testkonto, temporäre Testdaten): Ziehen im offenen Zeitfenster (08:00–16:00) erzeugte eine Teil-Buchung (08:45–12:30); Ziehen an einer bestehenden Schicht (09:00–13:00) öffnete das „Zeit ändern"-Formular vorausgefüllt (10:45–14:45), ohne die Liste/Schicht zu verändern, bis „Anfrage senden" geklickt würde — Formular ohne Absenden geschlossen, Testdaten vollständig aufgeräumt.

**security-reviewer-Durchlauf: 0 KRITISCH/HOCH/MITTEL, 2 NIEDRIG.** Kernbehauptungen des Plans am tatsächlichen Code/an der SQL-Migration verifiziert (nicht nur am Plantext): Trigger `calendar_slot_capacity_guard()` deckte Teil-Zeiträume schon vor S6 ab (generischer Bereichsvergleich, keine Exakt-Gleichheit) — keine neue Migration nötig, Behauptung korrekt. „Zeit ändern"-Vorschlag hat wirklich keine neue Schreib-Angriffsfläche (Grid-Handler rufen nur die Callback-Kette auf, kein Supabase-Import im Drag-Pfad). Freelancer-Gating ist serverseitig (`calendar_self_may_book()` unverändert, weiterhin `worker_type='freelancer' AND status='active'` plus Projektmitgliedschaft), nicht client-spoofbar. Bekannte, geerbte Einschränkung (Kapazitätsprüfung zählt Buchungs-Zeilen, nicht Zeit-Overlap) korrekt als Altlast charakterisiert, kein neuer Fund. Zwei NIEDRIG-Funde: (1) kein automatisierter Testfall, der `bookOwnShift()` direkt als Festangestellter aufruft, um die RLS-Ablehnung serverseitig zu erzwingen (nur UI-Ebene getestet, die RLS-Schranke wurde von mir per SQL-Lektüre bestätigt) — Empfehlung für einen künftigen Block, kein Fix hier; (2) Session-Timeout weiterhin ungedeckt (Bestandsfund, s. o.).

**Status: Freigegeben. Block „Schichtplan S6" damit abgeschlossen, bereit zum Commit.**

## Rechtsträger-Wechsel: AGB und Datenschutz auf die Calltalent LLC (24.08.2026)

Josips Auftrag: „AGB i privacy za salestalent.app moraju se prebaciti na Calltalent LLC." Rückfrage per `AskUserQuestion` geklärt: (1) `salestalent.app` bekommt Impressum-Äquivalent + Datenschutz + AGB; (2) **auch** die bestehenden Marketplace-Rechtsseiten wechseln auf die LLC — Calltalent Ltd. (UK, Company No. 16591113) ist damit nirgends mehr Vertragspartner im Produkt; (3) Anschrift von Josip nachgereicht: 1309 Coffeen Avenue STE 1200, Sheridan, Wyoming 82801.

**Ausgangsbefund (wichtig, weil der Auftragstext „umstellen" nahelegt, es gäbe schon Texte):** Auf `salestalent.app` gab es überhaupt keine Rechtstexte. `tenants.legal` war für alle Mandanten `{}` (per SQL geprüft), und die einzigen Rechtsseiten im Code lagen unter `src/app/marketplace/` — die sind per Host-Gate ausschließlich auf `marketplace.calltalent.ai` erreichbar. Es war also der offene Punkt 1 aus dem Abschnitt „salestalent.app — neuer Mandant für den internationalen Markt", nicht eine Textänderung.

**Erledigt:**

1. `src/lib/legal/company.ts` (NEU) — `LegalEntity` (zod) + `CALLTALENT_LLC` + `formatAddress()` + `resolveLegalEntity()`. Firmendaten stehen ab jetzt an genau EINER Stelle im Code; vorher lagen sie dreifach (de/en/bs) in den Sprachdateien und hätten bei diesem Wechsel an sechs Stellen gleichzeitig geändert werden müssen. Wyoming Filing ID fehlt noch → `registrationNumber: null`, der Registerabschnitt wird dann gar nicht gerendert (keine erfundene Nummer auf einer Rechtsseite).
2. `src/app/(legal)/` (NEU) — `layout.tsx` + `/legal-notice`, `/privacy`, `/terms` für Mandanten-Hosts. Deutsche Alias-Pfade `/impressum`, `/datenschutz`, `/agb` leiten per `redirect()` dorthin (bestehende deutsche Links laufen nicht ins Leere; auf dem Marketplace-Host schreibt die Middleware diese Pfade ohnehin vorher auf `/marketplace/...` um).
3. **Zugriffsgrenze:** Gerendert wird nur, wenn der Mandant einen Rechtsträger in `tenants.legal.entity` hat — sonst `notFound()`, in Layout UND Seite (ein Layout ist keine Sicherheitsgrenze). Damit kann auf der Domain eines White-Label-Kunden nie Calltalents eigenes Impressum erscheinen. `src/lib/tenant/types.ts` um `legal.entity` erweitert.
4. Inhalte in `messages/{de,en,bs}.json`, Namensraum `legal` (`shell`/`notice`/`privacy`/`terms`), Firmendaten als ICU-Platzhalter `{company}`/`{address}`/`{email}`. Der Datenschutztext bildet den tatsächlichen Verarbeitungsstand ab (Auftragsverarbeiter-Liste identisch mit Anlage 2 des AVV: Supabase/AWS Frankfurt, Bunny.net, Stripe, Resend, Anthropic, Voyage AI, Cloudflare) und hat einen eigenen Abschnitt zur Drittlandsübermittlung (Art. 44 ff. DSGVO) — der ist neu nötig, weil der Verantwortliche jetzt in den USA sitzt. AGB: Widerrufsbelehrung bleibt für EU-/EWR-Verbraucher drin, Rechtswahl Wyoming mit ausdrücklichem Vorbehalt zwingender Verbraucherschutzvorschriften (Art. 6 Abs. 2 Rom-I-VO).
5. Marketplace-Rechtsseiten (`impressum`/`datenschutz`/`agb`) + Shell-Fußzeile ziehen die Firmendaten jetzt aus `lib/legal/company.ts`; die Entity-Schlüssel (`companyName`, `companyNumber`, `address`) sind aus allen drei Sprachdateien entfernt. Neuer Abschnitt „Übermittlung in Drittländer" auch dort.
6. Auffindbarkeit: Login-Seite (Marken-Panel und eine eigene Zeile für Mobil, da das Panel unter `lg` ausgeblendet ist) und Sidebar-Fuß der Lernansicht verlinken die drei Seiten — jeweils nur, wenn der Mandant einen Rechtsträger hinterlegt hat, sonst wäre der Link eine Sackgasse.
7. **Datenbank (Supabase-MCP, Projekt `vklqksdiyiijzoirntyt`):** `tenants.legal.entity` für `salestalent` UND `calltalent` auf die LLC-Daten gesetzt (jsonb-Merge, bestehende Schlüssel unangetastet). `demo-blau` bewusst leer — der Testmandant belegt damit gleichzeitig den 404-Fall.
8. Tests: `src/lib/legal/company.test.ts` (NEU, 5 Fälle — u. a. „enthält weder ‚Ltd' noch die alte Company Number"), `e2e/legal-pages.spec.ts` (NEU, 3 Fälle: 404 ohne Rechtsträger, Rendern mit Rechtsträger inkl. fehlendem Registerabschnitt, Alias-Weiterleitungen — setzt den Rechtsträger für `demo-blau` selbst und räumt ihn wieder ab). `e2e/marketplace-public.spec.ts` angepasst: prüfte vorher „Calltalent Ltd." und „Company Number: 16591113", jetzt „Calltalent LLC"/„Sheridan, WY 82801".
9. Verifikation: `npx tsc --noEmit` → 0 Fehler. `npm run lint` → 0 Fehler. `npx vitest run` → 676 grün (davon 5 neue); die zwei roten Dateien (`env.test.ts`, `marketplace/fulfil.test.ts`) scheitern ausschließlich an der fehlenden `.env` in dieser Sitzungsumgebung (kein `NEXT_PUBLIC_SUPABASE_ANON_KEY`), nicht an dieser Änderung. `npx next build` → erfolgreich, alle sechs neuen Routen im Manifest. Playwright nicht gelaufen (kein Dev-Server/keine `.env` in dieser Umgebung, bekannter Blocker).
...
**Wyoming Filing ID gefunden und überall nachgetragen (24.08.2026, Josips Auftrag „Finde die Wyoming Filing ID"):** `2026-002057636`, eingetragen beim Wyoming Secretary of State am **17.08.2026, 8:16 Uhr** (Certificate of Organization). Quelle: `Articles Of Organization.pdf` im Google Drive des Auftraggebers — Web-Recherche war nicht möglich, der Egress-Proxy dieser Umgebung blockiert `wyobiz.wyo.gov`, `opencorporates.com` und Vergleichbares, und die Gmail-Suche liefert für dieses Konto durchgehend leere Ergebnisse. Registered Agent laut Urkunde: Company Sage Agents LLC, 1095 Sugarview Dr Ste 100, Sheridan, WY 82801 (nicht identisch mit der Geschäftsanschrift).

Nachgetragen in: `lib/legal/company.ts` (`registrationNumber`), `tenants.legal.entity` für `calltalent` und `salestalent` (Supabase), Website-Impressum (`Registereintrag`), Unternehmenszeile in Kontakt/Landingpages, `llms.txt` sowie AVV/TOM Fassung 2 (Kopfzeile und Vertragskopf). Dadurch rendern die Registerabschnitte auf `/legal-notice` und der Marketplace-Impressumsseite jetzt tatsächlich — vorher blendete sie die `registrationNumber === null`-Bedingung aus.

**Ablage der Word-Dateien:** Die aktualisierten `.docx` (mit Filing ID) wurden Josip direkt im Chat übergeben statt erneut nach Drive geladen — der Base64-Upload über den Drive-MCP schlug bei dieser Dateigröße zweimal mit „not a valid base64 string" fehl. Die `.md`-Zwillinge liegen aktualisiert in Drive; die Fassungen ohne Filing ID liegen im Drive-Papierkorb.
...
**1. Positionslogik erzeugte Duplikate (Auf/Ab-Buttons dauerhaft lahmgelegt):**

1. `count`→`max(position)+1` gefixt an allen sechs betroffenen Stellen: `createDraftCourse` (Kurse), `createCourseCategory` (Kategorien), `createModule`, `createSection`, `createLesson` in `src/lib/courses/actions.ts` sowie `upsertQuestion` (Insert-Zweig) in `src/lib/quiz/actions.ts` — exaktes Muster aus `src/lib/generator/apply.ts:104-111` übernommen (max(position) per `order(position desc).limit(1)`, `-1` als Startwert für die leere Tabelle).
2. Fehler beider Update-Aufrufe im Positionstausch werden jetzt geprüft (`moveModule`, `moveSection` in `courses/actions.ts`, `moveQuestion` in `quiz/actions.ts`) — vorher liefen zwei ungeprüfte, nicht-atomare `update()`-Aufrufe, ein stiller Teilausfall hätte zwei Zeilen dauerhaft dieselbe Position belassen können, ohne dass es irgendwo sichtbar wurde.
3. **NEUE Migration `supabase/migrations/20260907090000_reorder_swap_positions.sql`** (geschrieben, NICHT angewendet): drei `security definer`-RPCs (`swap_module_positions`, `swap_section_positions`, `swap_question_positions`), die den Tausch in EINE Datenbankanweisung ziehen (echte Atomarität statt nur Fehlerprüfung). **Bewusst NICHT in den App-Code verdrahtet** — die RPC existiert erst nach Josips `supabase db push`; sie jetzt schon aufzurufen hätte die aktuell funktionierenden Auf/Ab-Buttons bis dahin komplett lahmgelegt (Regel „nach jedem Feature lauffähiger Zustand"). Sobald angewendet: kleiner Folge-Commit, der die drei Update-Paare durch je einen `.rpc(...)`-Aufruf ersetzt.
4. `moveCourse` (Kursreihenfolge, `courses/actions.ts:348-374`) hat exakt dasselbe Zwei-Update-Muster wie `moveModule`, war aber nicht Teil des Auftrags (der nannte nur „Module/Sektionen/Lektionen/Fragen") — bewusst unverändert gelassen, hier vermerkt statt stillschweigend mitgefixt.
5. **Kein `moveLesson` gefunden** — Lektionen innerhalb einer Sektion sind im aktuellen Stand nicht per Auf/Ab sortierbar (nur Module, Sektionen und Fragen haben Move-Buttons, siehe `module-lesson-tree.tsx`/`quiz-editor.tsx`). Der `createLesson`-Positionsfix (Punkt 1) ist trotzdem nötig und behoben, betrifft aber nur die Erstzuweisung.
6. Regressionstest `src/lib/courses/actions.test.ts` (NEU, 2 Fälle) — reproduziert exakt das Auftragsszenario (Module A/B/C anlegen, B löschen, D anlegen) und beweist zusätzlich, dass `moveModule` danach weiterhin korrekt tauscht.

**2. Versuchslimit per Doppelklick umgehbar:**

1. **NEUE Migration `supabase/migrations/20260907091500_quiz_attempt_limit_rpc.sql`** (geschrieben, NICHT angewendet): `security definer`-RPC `submit_quiz_attempt` zieht Zählung+Insert in eine Datenbankanweisung (`insert ... select ... where (select count(*)...) < limit`, wie im Auftrag vorgeschlagen), zusätzlich mit `pg_advisory_xact_lock` auf (quiz_id, user_id) gegen echte Gleichzeitigkeit unter Standard-Isolationslevel. Leitet Mandant/Mitgliedschaft/`attempts_allowed` selbst aus der DB ab, `user_id` ausschließlich über `auth.uid()` — nie ein Client-Parameter.
2. `src/lib/quiz/actions.ts` `submitAttempt()` **ist bereits auf diese RPC umgestellt** (anders als der Positionstausch oben, hier war „Fix: ... zusammenziehen" eine verbindliche Auftragsvorgabe, kein optionaler Vorschlag) — ruft `supabase.rpc("submit_quiz_attempt", ...)` statt des vorherigen `admin.from("attempts").insert(...)`. **Wichtiger Hinweis für Josip/tester:** Bis diese Migration angewendet ist, schlägt jeder Prüfungsabschluss mit „Speichern fehlgeschlagen: …" fehl (die RPC existiert in der Live-DB noch nicht) — Reihenfolge beim nächsten `supabase db push` beachten, idealerweise vor dem nächsten Deploy dieses Codestands.
3. Bei erreichtem Limit liefert die Funktion weiterhin die bestehende deutsche Meldung „Versuchslimit erreicht." (unverändert gegenüber vorher, keine neue i18n-Zeichenkette nötig).
4. Die frühere, ungeprüfte Vorab-Zählung bleibt als schneller, nicht-durchsetzender UX-Vorab-Check erhalten (spart die Fragen-/Bewertungsarbeit im Normalfall) — durchgesetzt wird das Limit ausschließlich noch von der RPC.
5. Regressionstest `src/lib/quiz/actions.test.ts` (NEU, 3 Fälle) — reproduziert den Doppelklick-Fall (`Promise.all` mit zwei nahezu gleichzeitigen `submitAttempt()`-Aufrufen bei `attempts_allowed=1`): genau ein Aufruf erfolgreich, genau eine `attempts`-Zeile. Dazu je ein Kontrollfall ohne Limit (beide dürfen) und mit zwei verschiedenen Nutzern (teilen sich das Limit nicht).

**3. Quizfragen wurden nach dem Absenden neu gemischt:**

1. `src/components/learn/quiz-runner.tsx`: Mischen hing an `isRunning` (`phase === "running"`), das sich bei JEDEM Phasenwechsel ändert — auch beim Übergang „läuft" → „Ergebnis". Fix: neuer State `attemptRunCount`, ausschließlich in `handleStart()` erhöht, als einzige `useMemo`-Abhängigkeit für das Mischen. Serverseitige Bewertung war nie betroffen (arbeitet über `questionId`).
2. **Kein Test ergänzt** — im Projekt gibt es kein Component-Test-Setup (siehe Kopfkommentar `src/lib/marketplace/redirect.ts`), ein neues Test-Framework einzuführen war explizit nicht Teil des Auftrags. Der Fix ist rein strukturell (eine `useMemo`-Abhängigkeit ersetzt) und durch Code-Review nachvollziehbar.
...
Josips Auftrag per Swarm-Kommando. Ergebnis: `PROJEKTANALYSE_2026-09-08.md` (809 Zeilen) auf Branch `claude/projekt-analyse-verbesserungen-2498k4`, zuletzt Commit `2713b89`.

**Vorgehen:** Baseline auf `main` (tsc 0, ESLint 0, Vitest 694 grün), Live-Abgleich per Supabase- und Cloudflare-MCP (nur lesend), drei ungemergte Branches gelesen. Der Lauf wurde am 09.09. auf Josips Auftrag „Wiederhole den Agentenlauf" vollständig wiederholt und nach jedem Nutzungslimit fortgesetzt: 16 lesende Bereichs-Prüfer, je ein Gegenprüfer, ein Vollständigkeits-Kritiker und eine Nachrunde über die zwei von ihm benannten Lücken (Browser-Medienpipeline, Fehler- und Ausnahmezustände). Kritische und hohe Befunde hat der Orchestrator zusätzlich selbst am Code nachvollzogen.
