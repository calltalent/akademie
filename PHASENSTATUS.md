...
**Noch offen:**
1. Manueller Test: im Admin ein Quiz an einem Quiz-Block anlegen (Titel, Bestehensgrenze, Versuche), je eine Frage pro Typ (`single`/`multi`/`gap`/`open`) anlegen, dann als Lernender die Lektion aufrufen, Quiz starten, beantworten, abschicken — prüfen ob Ergebnis/Bestanden-Status korrekt, ob `open`-Antwort das Ergebnis nicht verfälscht.
2. Versuchslimit testen: `attemptsAllowed` z. B. auf 1 setzen, zweimal versuchen — zweiter Versuch sollte serverseitig blockiert werden (auch bei manipulierter Client-Anfrage, da `submitAttempt` selbst erneut zählt).
3. Bestehensgrenze exakt testen (z. B. `pass_pct=70`, Score genau 70% erreichen → sollte „Bestanden" zeigen, `>=`-Vergleich).
4. Prüfen, dass ein Lernender ohne Mitgliedschaft/nicht angemeldet weder `loadQuizForLearner` noch `submitAttempt` etwas liefert (sollte durch RLS + Server-Checks bereits abgedeckt sein).
5. Git-Commit (`feat: Block 2 - Quiz/Pruefungen und Versuche`).
6. Übergabe an `tester`-Agent für Playwright-Lauf (E2E: Quiz anlegen → Lernender absolviert → Ergebnis).
