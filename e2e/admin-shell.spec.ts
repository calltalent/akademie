...
test("Admin-Bereich zeigt die neue gruppierte Sidebar-Navigation", async ({ page }) => {
  ...
  await expect(nav.getByRole("link", { name: "Zur Akademie" })).toBeVisible();

  // Übersicht ist die aktuelle Seite -> aria-current="page".
  await expect(nav.getByRole("link", { name: "Übersicht" })).toHaveAttribute("aria-current", "page");
});
