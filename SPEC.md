...
**Umsetzung in Blöcken:** S1 (07.08.2026, Grunddatenmodell, Feature-Flag, Arbeiter-/Projektverwaltung, lesende Wochenansicht, Ein-/Ausstempeln) und S2 (08.08.2026, Schicht-CRUD durch Admin, Zeitfenster-Verwaltung mit Serienanlage, Freelancer-Selbstbuchung, Abwesenheiten/Feiertagsimport, Freelancer-Sollstunden-Selbstverwaltung) umgesetzt und live. S3 (Änderungsanfragen-UI, Projektleiter-Zugang zum Admin-Bereich) und S4 (KI-Schichtplanung) sind Roadmap, jeweils mit eigenem `architect`-Durchlauf vor der Umsetzung. **Korrektur (08.08.2026):** die S1-Annahme „Schema und RLS für alle vier Blöcke liegen bereits vollständig in der S1-Migration" traf für S2 nur teilweise zu — die Selbstbuchungs-Policy auf `calendar_shifts` und der Spaltenschutz-Trigger für die Freelancer-Sollstunden-Selbstverwaltung fehlten und wurden in `supabase/migrations/20260808100000_shift_calendar_s2.sql` nachgezogen. Zusätzlich behob `supabase/migrations/20260808110000_shift_calendar_guard_tenant_fix.sql` einen bei diesem S2-Review gefundenen, systemischen Sicherheitsfund (Spaltenschutz-Trigger prüfte `new.tenant_id`/`new.worker_id` statt `old.*`, betraf auch das S1-Pendant `calendar_time_entries_guard()`).
