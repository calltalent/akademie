 ...
 * Zeilen weder Fehler noch Zeilenzahl" (`src/lib/calendar/ai/holidays/
 * process.ts`), identische Stelle wie `src/lib/generator/process.ts` (siehe
...
const { adminClientRef, enforceQuotaMock, updateAiJobMock, callHolidayResearchModelMock } = vi.hoisted(() => {
  const adminClientRef: { current: unknown } = { current: null };
  const enforceQuotaMock = vi.fn();
  const updateAiJobMock = vi.fn();
  const callHolidayResearchModelMock = vi.fn();
  return { adminClientRef, enforceQuotaMock, updateAiJobMock, callHolidayResearchModelMock };
});
...
vi.mock("@/lib/calendar/ai/holidays/pipeline", () => ({
  callHolidayResearchModel: (...args: unknown[]) => callHolidayResearchModelMock(...args),
}));

import { processNextHolidayResearchJob } from "./process";

beforeEach(() => {
  enforceQuotaMock.mockReset();
  updateAiJobMock.mockReset();
  callHolidayResearchModelMock.mockReset();
});

describe("processNextHolidayResearchJob — CAS-Sperre (verifizierter Fehler, behoben)", () => {
  it("gibt processed:false zurück und startet KEINEN KI-Aufruf, wenn ein anderer Prozess den Job zwischenzeitlich bereits übernommen hat", async () => {
    ...
    const result = await processNextHolidayResearchJob();
    ...
    expect(callHolidayResearchModelMock).not.toHaveBeenCalled();
