...
import { QUIZ_KINDS, type Question, type QuizKind } from "@/lib/quiz/schema";
...
export function QuizEditor({
  ...
  const t = useTranslations("admin.courseEditor.quiz");
  const tCommon = useTranslations("common");
  const tKinds = useTranslations("quiz.kinds");
  const tQuestionKinds = useTranslations("quiz.questionKinds");
  const tPoints = useTranslations("quiz");
  ...
  return (
    <div className="flex flex-col gap-8">
      <section className="rounded-md border p-4" style={{ borderRadius: "var(--radius)" }}>
        <h2 className="mb-3 text-lg font-medium">{t("metaHeading")}</h2>
        <form action={metaAction} className="flex flex-col gap-3">
          ...
          {metaState.success && !metaState.error && <p className="text-sm text-green-700">{t("savedNotice")}</p>}
      ...
      <section>
        <h2 className="mb-3 text-lg font-medium">{t("questionsHeading", { count: initialQuestions.length })}</h2>
        <ul className="flex flex-col gap-3">
            ...
            <li key={q.id} className="rounded-md border p-3" style={{ borderRadius: "var(--radius)" }}>
              {editingQuestionId === q.id ? (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-xs font-medium text-gray-500">{tQuestionKinds(q.kind)}</p>
                    <p className="text-base">{q.prompt || t("noPromptText")}</p>
                    <p className="text-xs text-gray-500">{tPoints("pointsLabel", { points: q.points })}</p>
                  </div>
          ...
          {initialQuestions.length === 0 && (
            <p className="text-sm text-gray-500">{t("noQuestionsText")}</p>
          )}
        </ul>
